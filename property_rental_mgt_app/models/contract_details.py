...
class ContractDetails(models.Model):
    ...
    def action_view_pdc_payments(self):
        ...
        action = self.env.ref('property_rental_mgt_app.account_pdc_payment_payable_menu_action').read()[0]
        ...
        if len(pdc_payments) > 1:
            ...
            action['views'] = [
                (self.env.ref('property_rental_mgt_app.account_payment_inherit_pdc_form_view').id, 'form')]
